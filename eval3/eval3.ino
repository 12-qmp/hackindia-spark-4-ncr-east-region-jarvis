...
void mqttCallback(char* topic,byte* payload,unsigned int len){
  ...
  // BUG FIX: many MQTT clients append \n or \r\n to payloads,
  // causing msg=="ON" to silently fail — trim whitespace first
  msg.trim();
  Serial.print("[MQTT] Topic: "); Serial.print(topic);
  Serial.print(" | Payload: '"); Serial.print(msg); Serial.println("'");
  ...
  for(int i=0;i<4;i++){
    if(String(topic)==baseTopic+"/relay/"+String(i+1)){
      ...
      Serial.print("[MQTT] Relay "); Serial.print(i+1);
...
void connectMQTT(){
  ...
  if(mqtt.connect(deviceID.c_str())){
    ...
    // Print exact topics so you always know what to publish to
    Serial.println("=== MQTT TOPICS ===");
    Serial.println("Relay 1 : " + baseTopic + "/relay/1  payload: ON / OFF");
    Serial.println("Relay 2 : " + baseTopic + "/relay/2  payload: ON / OFF");
    Serial.println("Relay 3 : " + baseTopic + "/relay/3  payload: ON / OFF");
    Serial.println("Relay 4 : " + baseTopic + "/relay/4  payload: ON / OFF");
    Serial.println("PIR     : " + baseTopic + "/pir      payload: ON / OFF");
    Serial.println("===================");
  }
}
...
void setup(){
  ...
  if(digitalRead(WIFI_BTN)==LOW){
    WiFi.disconnect(true);   // purani STA connection tod do
    WiFi.mode(WIFI_AP);      // sirf AP mode, STA bilkul band
    ...
    // FIX: flush response to client before rebooting, reduced delay to 200ms
    server.on("/savewifi",[](){ saveWiFi(server.arg("s"),server.arg("p")); server.send(200,"text/html","Saved. Rebooting..."); server.handleClient(); delay(200); ESP.restart(); });
...
/* ===================== LOOP ===================== */
void loop(){
  // AP mode: web server + IR only — skip WiFi/MQTT/PIR automation overhead
  if(apMode){
    server.handleClient();

    // IR CONTROL in AP mode (no MQTT publish since not connected)
    ...
    return; // WiFi/MQTT/PIR automation abhi bhi skip — sahi hai
  }
  ...
  /* IR CONTROL (STA mode — MQTT sync bhi hoga) */
  if(IrReceiver.decode()){
    uint32_t c=IrReceiver.decodedIRData.decodedRawData;

    if(c==IR_R1){
      relayState[0]=!relayState[0];
      digitalWrite(relayPin[0],relayState[0]?LOW:HIGH);
      // IR→MQTT sync: publish new relay 1 state (retained)
      if(mqtt.connected()) mqtt.publish((baseTopic+"/relay/1").c_str(), relayState[0]?"ON":"OFF", true);
    }
    if(c==IR_R2){
      relayState[1]=!relayState[1];
      digitalWrite(relayPin[1],relayState[1]?LOW:HIGH);
      // IR→MQTT sync: publish new relay 2 state (retained)
      if(mqtt.connected()) mqtt.publish((baseTopic+"/relay/2").c_str(), relayState[1]?"ON":"OFF", true);
    }
    if(c==IR_R3){
      relayState[2]=!relayState[2];
      digitalWrite(relayPin[2],relayState[2]?LOW:HIGH);
      // IR→MQTT sync: publish new relay 3 state (retained)
      if(mqtt.connected()) mqtt.publish((baseTopic+"/relay/3").c_str(), relayState[2]?"ON":"OFF", true);
    }
    if(c==IR_R4){
      relayState[3]=!relayState[3];
      digitalWrite(relayPin[3],relayState[3]?LOW:HIGH);
      // IR→MQTT sync: publish new relay 4 state (retained)
      if(mqtt.connected()) mqtt.publish((baseTopic+"/relay/4").c_str(), relayState[3]?"ON":"OFF", true);
    }
    if(c==IR_PIR){
      pirEnabled=!pirEnabled;
      digitalWrite(PIR_LED,pirEnabled);
      // IR→MQTT sync: publish new PIR enabled state (retained)
      if(mqtt.connected()) mqtt.publish((baseTopic+"/pir").c_str(), pirEnabled?"ON":"OFF", true);
    }
